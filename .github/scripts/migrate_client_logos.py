...
# Current brands shown in Cáliber's real "Parceiros & Clientes" section.
CLIENTS = [
    ("Claro", "claro", ["claro"]),
    ("NET", "net", ["net"]),
...
def discover_sources() -> dict[str, list[str]]:
    ...
    response = requests.get(REFERENCE_URL, headers=page_headers, timeout=60)
        ...
        for img in images:
            ...
            for alias in aliases_n:
                for field in fields:
                    if field == alias:
                        score = max(score, 100)
                    elif alias and alias in field:
                        score = max(score, 80)
                    elif alias and all(token in field for token in alias.split()):
                        score = max(score, 60)
            urls = image_urls(img)
            if score and urls:
                ranked.append((score, urls))
        if ranked:
            ranked.sort(key=lambda item: item[0], reverse=True)
            urls: list[str] = []
            for _score, group in ranked:
                urls.extend(group)
            result[slug] = list(dict.fromkeys(urls))
            print(f"DISCOVERED {display}: {len(result[slug])} candidate URL(s); top={result[slug][0]}")
        else:
            print(f"NOT FOUND IN PAGE: {display}")
    return result
...
def expand_url(url: str) -> list[str]:
    ...
    if host in {"i0.wp.com", "i1.wp.com", "i2.wp.com", "i3.wp.com"} and path.startswith("/calibergestao.com.br/"):
        ...
        origin = f"https://calibergestao.com.br{relative}"
        urls.append(origin)
        ...
        for wp_host in ("i0.wp.com", "i1.wp.com", "i2.wp.com", "i3.wp.com"):
            ...
            if fit:
                urls.append(add_query(base, {"fit": fit, "ssl": "1"}))
            if width:
                urls.append(add_query(base, {"w": width, "ssl": "1"}))
            if width and height:
                urls.append(add_query(base, {"resize": f"{width},{height}", "ssl": "1"}))
                urls.append(add_query(base, {"w": width, "h": height, "ssl": "1"}))
    return list(dict.fromkeys(urls))
...
def fetch_image(session: requests.Session, urls: list[str], label: str) -> tuple[bytes, str] | None:
    candidates: list[str] = []
    for url in urls:
        candidates.extend(expand_url(url))
    ...
    for round_no in range(1, 5):
        for index, candidate in enumerate(candidates):
            try:
                ...
                response = session.get(candidate, headers=headers, timeout=25, allow_redirects=True)
...
def transparentize_edge_background(image: Image.Image) -> Image.Image:
    ...
    px = rgba.load()
    border = [px[x, y] for x in range(width) for y in (0, height - 1)]
    border += [px[x, y] for y in range(height) for x in (0, width - 1)]

    if min(p[3] for p in border) < 250:
        return rgba

    channels = list(zip(*[(p[0], p[1], p[2]) for p in border]))
    bg = tuple(sorted(c)[len(c) // 2] for c in channels)
    deviations = sorted(color_distance(p, bg) for p in border)
    p90 = deviations[int((len(deviations) - 1) * 0.90)]
    neutral_light = max(bg) - min(bg) <= 50 and min(bg) >= 115
    uniform = p90 <= 32
    if not (neutral_light or uniform):
        return rgba

    threshold = 78 if neutral_light else 46
    ...
    def add(x: int, y: int) -> None:
        ...
        seen[idx] = 1
        r, g, b, a = px[x, y]
        if a <= 25 or color_distance((r, g, b), bg) <= threshold:
            queue.append((x, y))
...
def process_asset(raw: bytes, destination: Path, label: str) -> None:
    ...
    source.load()
    image = transparentize_edge_background(source)
    alpha = image.getchannel("A")
    alpha_min, alpha_max = alpha.getextrema()
    if alpha_max == 0 or alpha_min == 255:
        raise RuntimeError(f"{label}: source has no safe true transparency after background treatment")
    bbox = alpha.point(lambda a: 255 if a >= 12 else 0).getbbox()
    if not bbox:
        raise RuntimeError(f"{label}: no visible pixels")
    ...
    corners = [verify.getpixel((0, 0))[3], verify.getpixel((verify.width - 1, 0))[3], verify.getpixel((0, verify.height - 1))[3], verify.getpixel((verify.width - 1, verify.height - 1))[3]]
    if amin != 0 or amax == 0 or any(corners):
        raise RuntimeError(f"{label}: final alpha validation failed")
    print(f"ASSET OK {label}: source={source.format} {source.width}x{source.height}; final={verify.width}x{verify.height}; alpha={amin}-{amax}")
  ...
  return (
    <section className={{`client-logos ${{className}}`}} aria-label="Parceiros e clientes da Cáliber">
      ...
      <div className="client-logos-viewport">
        <div className="client-logos-track"><LogoGroup /><LogoGroup clone /></div>
      </div>
    </section>
  );
}}
...
for display, slug, _aliases in CLIENTS:
    ...
    if not fetched:
        print(f"SKIP unavailable source: {display}")
        continue
    raw, _source_url = fetched
    try:
        process_asset(raw, OUT / f"{slug}.png", display)
        successes.append((display, slug))
    except Exception as exc:
        print(f"SKIP unsafe asset {display}: {exc}")

if len(successes) < MIN_LOGOS:
    raise RuntimeError(f"Only {len(successes)} valid real logos recovered; need at least {MIN_LOGOS}")

write_component(successes)
write_css()
cleanup({slug for _name, slug in successes})
