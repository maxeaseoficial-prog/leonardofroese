...
import { loadLiveDiagnostic, saveLiveDiagnostic } from "@/features/lucro-2x/live-session";
...
export const Route = createFileRoute("/raiox/dados")({
  head: () => funnelHead,
...
function DataCapture() {
  ...
  useEffect(() => setContact(loadLiveDiagnostic().contact), []);
  ...
  const submit = (event: FormEvent) => {
    ...
    if (saved.answers.length < 12) {
      navigate({ to: "/raiox/perguntas" });
      return;
    }
    const next = { ...saved, contact };
    saveLiveDiagnostic(next);
    submitDiagnostic({
      data: { contact, answers: next.answers, utm: next.utm, source: "live" },
    }).catch((error) => console.error("Falha ao notificar diagnóstico", error));
    navigate({ to: "/raiox/resultado" });
  };
  return (
    <FunnelShell>
