...
export const Route = createFileRoute("/raiox/perguntas")({
  head: () => funnelHead,
  component: Questions,
});
...
function Questions() {
  ...
  const back = () => (index === 0 ? navigate({ to: "/raiox" }) : setIndex((value) => value - 1));
  const choose = (score: number) => {
    const next = answers.slice(0, index);
    next[index] = score;
    setAnswers(next);
    saveLiveDiagnostic({ ...loadLiveDiagnostic(), answers: next });
    window.setTimeout(
      () => (index === 11 ? navigate({ to: "/raiox/dados" }) : setIndex(index + 1)),
